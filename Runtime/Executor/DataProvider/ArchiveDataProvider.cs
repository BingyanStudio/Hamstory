    ...
    public class ArchiveDataProvider : DataProvider
    {
        public override bool Predicate(StoryExecutorBase executor, string expression)
        {
            ...
            if (TryPredicate(expression, ">=", (l, r) =>
                {
                    if (!Compare(l, r, out var result)) return false;
                    return result >= 0;
        ...
        public override string Serialize(StoryExecutorBase executor, string content)
        {
            ...
            while ((l = content.IndexOf('{', idx)) != -1)
            {
                ...
                {
                    sb.Append(content.Substring(idx, l - idx));
                    sb.Append(Get(content.Substring(l + 1, r - l - 1)));
                    idx = r + 1;
                }
            }
            sb.Append(content.Substring(idx, content.Length - idx));
            return sb.ToString();
        }
        ...
        private bool Compare(object l, object r, out int result)
        {
            result = 0;
        ...
        private object ParseVar(string key)
        {
            key = key.Trim();
        ...
        private object Get(string key) => Archive.Get<object>(key, null);
    }
}